...
| **Headless suite** | `tools/test_contract.rb` **477, 0** · `tools/test_appliances.rb` **68, 0** · `tools/test_appliance_seam.rb` **30, 0**. The end-panel work added **13**: the three collections and their code count, contract validity, height-from-the-row, the depth label the catalog contradicts itself about, no front, the width refusal, the one misprinted height, the refusal to draw on a guessed ground, the 1,8 cm surcharge that must never become an article, and a picker label for every class. **Run under the Ruby macOS ships (2.6) by the commit script, which will not proceed otherwise.** |
