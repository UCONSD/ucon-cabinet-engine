...
| | |
|---|---|
| **HEAD** | **`98a511c`** — *feat(appliances): one optional seam that checks a drawn niche against the published opening (0.78.0)*, re-read from `refs/heads/main` 2026-08-25, 16:0x. Under it: `ed91383`, `862ed5f`, `1b44ca6`, `27ede4c`. **The last commit that changed BEHAVIOUR is `98a511c` itself (0.78.0)** — it carries `core/88_appliance_check.rb` and the version bump; before it the last behaviour change was `e9229d0` (0.77.2). |
| **Pushed** | **Do not trust this cell — read the two refs** (rule 16, and it is two `cat`s). On 2026-08-25, 16:0x, `refs/heads/main` and `refs/remotes/origin/main` were both `98a511c` — pushed, working tree clean. (At the close of 2026-08-24 they were both `27ede4c`.) |
| **Working tree** | **2026-08-25, 16:0x: CLEAN.** The five files this cell listed as uncommitted — `core/00_version.rb` (0.78.0), `core/88_appliance_check.rb`, `tools/test_appliance_seam.rb`, `claude/findings-2026-08-25-appliance-seam.md` and this file — went in as `98a511c`. Written through the device bridge by a Cowork session; **Andriy commits in his own terminal.** A file that records HEAD cannot name the commit carrying its own text, so it sits **one** commit behind by construction. **ONE IS STRUCTURAL. MORE IS NEGLECT.** That sentence has now been used as cover twice: the HEAD row drifted to FOURTEEN commits behind on 2026-08-24, and on 2026-08-25 evening it went stale again while this very cell was being written. **If the sha above is not the sha in `refs/heads/main` or its immediate parent, stop and rewrite this block before doing anything else.** Do not guess forward; read the refs and the log. |
