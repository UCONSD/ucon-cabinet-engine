...
| **Worktops** | **THE ENGINE DOES NOT DRAW THEM. Andriy draws the stone; the engine STAMPS the article onto it** — decided 2026-08-28 after three refusals in a row came out of the model rather than out of the suite. The reason is this project's own discipline turned on itself: every rule that generated a worktop shape would have been **inferred from one kitchen**, and the rules would never have ended — mitres, scribes, 45° returns, angled walls, a notch round a column. **Do not re-add a generator without re-opening this.** `core/62_top_stamp.rb` is pure and holds all of it: the thickness is the smallest of the three dimensions (no axis convention to remember, survives 45°), the band Andriy chose decides which of the other two is the depth (sorting would call a 300×650 return piece '650 long'), three refusals — a thickness that is not the article's, a piece over 3140, a piece deeper than its band — and one remark, for stone cut down from a wider band, which is what a 644,5 run buys because no 620 band exists. Tolerance **1 mm everywhere**: hand geometry lands on fractions and a refusal at 0,3 would be about SketchUp. **The one assumption is Elda Q28** and it is written onto every object it touches: a mitred piece has no single length, so the order figure is the piece's BOUNDING RECTANGLE, the sheet it is cut from. **What was lost, and it is owed back:** a stamped top does not know the run beneath it, so nothing checks the stone covers the cabinets — `build_worktop` knew, because it measured them. It keeps its code and its checks and **lost its button**; two ways to make a worktop are two ways to be wrong. **The sink is two facts** (`core/64_sink_mark.rb`): the ORDER fact is a variant on the top (p.110 prices a bowl each, no code of its own), the DRAWING fact is a dashed rectangle 1 mm above the stone that cuts nothing — position is not in the order, which is what lets them come apart. And it **refuses where there is no stamped top**, which is the only covering check the model has left. **STAMPED, 2026-08-30 — six pieces, and the tops of 545 Avenida Primavera are an ORDER.** All `TOPDR008040`, group **D**, finish **Marmorio**, `code_status` PRELIMINARY: west run 3080 @650, south leg 1903 @650, east of the range 874,3 @650, the angled-wall wedge 102 @650, island 2444 **@700** (663 deep, 650 refuses, 88 points paid knowingly), and the ledge behind the island 2398 **@380** — the only piece in the kitchen whose band fits with nothing paid for. **The sink came with the stamp**, exactly as designed: it had refused all week over an unstamped top and placed itself the moment the west run carried one. **The cut of the west run is option A — Andriy's, because it was already drawn**; three schemes were costed and sit one per cent apart, so the joint and not the price decided it, and the price of A is the 102 x 152 wedge as its own order line. **PARKED, AND NOT A TYPO: four pieces carry `visible_side_edge = 1` where the stated rule gives 0** — 569 points on a stone of about 10 580. Left in the model deliberately, because the fix is not the four numbers: **the stamp asks ONE edge count of a WHOLE SELECTION while an edge belongs to ONE END of ONE PIECE.** Next session gets a separate edge stamp or a rule that survives a shared dialog. **And printed p.172 now owes this kitchen TWO priced facts**, not one: the sink cutout it already owed, and a finished long edge — the ledge is finished on three sides and p.110 prices only ENDS, capped at two. Account: `claude/findings-2026-08-30-the-stone-stamped.md`. **NOT AN ORDER, AND THIS ROW SAID OTHERWISE UNTIL 2026-08-31.** Whether the worktop is bought on this project AT ALL is UNDECIDED (Andriy, 2026-08-31). Dated and added rather than corrected in place, learned rule 9: everything above happened and is recorded truthfully - six pieces, the cut, the sink, the four parked edges - and what is not settled is one level up, whether any of it becomes a purchase. The stone work is STOPPED until that decision exists, and while it is stopped the four `visible_side_edge = 1` cost nothing, which is why debt 1 stopped blocking. The edge procedure was designed before the stop and is recorded, unbuilt: two questions per piece, one per END; the edge question leaves the stone stamp entirely; the ledge's long edge waits for printed p.172. AND A LIVE DEFECT FOUND WHILE DESIGNING IT: the stone stamp rebuilds `variants` from scratch, so a SECOND stamp over a piece deletes the sink bowl variant that `SinkMark` wrote - the sink merges, the stamp does not. Nothing is broken in the model today; the agreed design is what would have made it certain. Account: `claude/findings-2026-08-31-the-plinth-nobody-owned.md`. |
