...
| | |
|---|---|
| **HEAD** | **`e01f2ef`** — *docs(appliances): phase 2 finally ran in SketchUp — five findings, and a range that reserves nothing*, re-read from `refs/heads/main`. Under it: `12f31a7` (two extensions, one repository), `84d9920`, `98a511c`. **Core is still 0.78.0** — nothing since `98a511c` changed what the engine draws. |
| **Pushed** | **YES — both refs are `e01f2ef`.** **Do not trust this cell — read the two refs** (rule 16, and it is two `cat`s). |
| **Working tree** | **NOT THIS FILE ONLY, AND THE OLD SENTENCE WAS WRONG ABOUT WHY.** Uncommitted on 2026-08-25 evening: the reserved-void work (`src/ucon_cabinet_engine/core/` 00/20/60/70/80/85, `docs/Reserved_Void_Spec_v0.1.md`, `registry/cesar/tall_h210_base78.json`, `registry/cesar/_manifest.json`, `tools/test_contract.rb`, two new `claude/` notes) **plus a deletion that has been sitting unstaged**: `_to_delete/00_version.rb.bak.2026-08-23` and `_to_delete/probe_tmp.rb` are TRACKED IN `HEAD`, the directory is gone from disk, and the deletion was never committed. **The cell below called `_to_delete/` untracked and per-machine; it is neither.** The structural one-commit lag applies to THIS FILE and to nothing else — it was covering two real deletions. |
| **Core version** | **0.79.0 uncommitted** (0.78.0 at `HEAD`) — it changes what is DRAWN. Shell 0.6.0. **Appliances 0.1.1** — a SEPARATE clock, on purpose; see `claude/appliance-rules-decided.md` §11 |
| **Headless suite** | **THREE suites, each runnable alone.** `tools/test_contract.rb` **440 checks, 0 failures** (was 433; +7 for the reserved void). `tools/test_appliances.rb` **54, 0**. `tools/test_appliance_seam.rb` **16, 0**. Re-run 2026-08-25 evening on the office Mac through the bridge, Ruby 3.0.2, after the void work. **The 2.6 run has still not been done since `848f10f`.** They stay unmerged: a suite needing both trees could not prove either stands alone. |
...
| **Object Contract** | **v2.1**, and **one of the two named gaps is closed**. A front whose HEIGHT comes from the opening is now expressible as a `remainder` entry in the front stack — `docs/Reserved_Void_Spec_v0.1.md`, `object_class: void`, key `void_role`. **printed p.414's gap is the same shape on the WIDTH axis and is NOT closed.** |
| **Registry** | **724 codes** in **44 section files** carrying **28 catalog sections** (was 711; +13 from printed p.121-125). `catalog_map` holds 67 sections. |
| **`_to_delete/`** | **CORRECTED 2026-08-25: it was TRACKED, not untracked.** Two files live in `HEAD` and are deleted from disk without a commit. The previous version of this cell said the directory was 'not part of the repository's state at all' — it was, and saying otherwise is what let the deletion sit. Andriy commits it with an explicit file list; the bridge cannot. |
