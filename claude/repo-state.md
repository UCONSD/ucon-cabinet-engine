...
| | |
|---|---|
| **HEAD** | **`12f31a7`** — *feat(appliances): two extensions, one repository — the package moves in, the design is vendored, and a hand-edited copy fails its own suite*, re-read from `refs/heads/main` 2026-08-25 evening. 26 files, +3682 / −30. Under it: `84d9920` (the Project housekeeping), `98a511c` (the seam, 0.78.0). **Core is still 0.78.0** — `12f31a7` moved files and added a design kit, it did not change what the engine draws. |
| **Pushed** | **YES — `refs/heads/main` and `refs/remotes/origin/main` are both `12f31a7`.** `84d9920` had sat local for a while and went up with it. **Do not trust this cell — read the two refs** (rule 16, and it is two `cat`s). |
| **Working tree** | **2026-08-25 evening: THIS FILE ONLY.** Everything else went in as `12f31a7`. A file that records HEAD cannot name the commit carrying its own text, so it sits **one** commit behind by construction. **ONE IS STRUCTURAL. MORE IS NEGLECT.** That sentence has been used as cover twice — fourteen commits behind on 2026-08-24, and stale again on 2026-08-25 while this very cell was being written. **If the sha above is not the sha in `refs/heads/main` or its immediate parent, stop and rewrite this block before doing anything else.** Do not guess forward; read the refs and the log. |
