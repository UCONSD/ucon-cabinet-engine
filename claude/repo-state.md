...
## As of 2026-08-25, late evening (third pass — B6, the run gap) — three rows AMENDED 2026-08-26 morning, then AGAIN 2026-08-26 afternoon (owed 10)

| | |
|---|---|
| **HEAD** | **`063c373`** — *docs(repo-state): the four commits are pushed*, read 2026-08-26 morning from `refs/heads/main` on the office Mac. **It is DOCS ONLY: the code at `HEAD` is `da758d9`** — *a gap in the run is a gap in the FINISHED run … (0.81.0)*. The previous version of this row said `95c9297`, which was two commits stale within the hour it was written. **Read the ref.** |
| **Pushed** | **NO — one commit is unpushed, MEASURED 2026-08-26 morning.** `refs/heads/main` is **`063c373`**; `refs/remotes/origin/main` is **`da758d9`**. Four commits did go out in one push (`e01f2ef..da758d9`) — and then `063c373`, the commit whose own message says *the four commits are pushed*, was made on top of that push and never sent. **Rule 16 catching its own author: a cell that says PUSHED is a report about the moment it was written.** Working tree clean at this reading. |
| **Working tree** | **NOT CLEAN, 2026-08-26 afternoon.** Eleven modified files and one new one, all of them owed 10 and the live run that followed it: `registry/cesar/usa_tall_h210.json`, `core/00_version.rb`, `core/60_generator.rb`, `core/88_appliance_check.rb`, `core/90_palette.rb`, `tools/test_contract.rb`, `tools/test_appliance_seam.rb`, `docs/Elda_Open_Questions_v0.1.md`, `claude/appliance-rules-decided.md`, `claude/findings-2026-08-25-appliance-seam.md`, this file, and TWO new files, each needing a `git add` of its own: `claude/findings-2026-08-26-seam-findings-closed.md` and `claude/findings-2026-08-26-east-fridge.md`. **Read it from `git`, never from this cell** — through the device mount `git status` can lag a write by a second. |
| **Core version** | **0.82.0 UNCOMMITTED** (0.81.0 at `HEAD`, committed in `da758d9`). It moves a DATUM and draws a new body: the USA appliance housing starts on the floor instead of on the plinth, and the remainder above it gets a filler. Shell **0.6.0** (`main.rb`). **Appliances 0.2.0 — UNTOUCHED, and there is no `.rbz` to rebuild in this change**: `void`, `opening_h` and `matches_niche?` all predate the installed package. Two clocks, and only one moved (§11). |
| **Headless suite** | **THREE suites, each runnable alone.** `tools/test_contract.rb` **464, 0** (459 before owed 10). `tools/test_appliances.rb` **68, 0** — untouched by owed 10. `tools/test_appliance_seam.rb` **30, 0** (25). Run 2026-08-26 afternoon on the office Mac through the device bridge, **Ruby 3.0.2 on the bridge's Linux VM, which is not macOS**. **The 2.6 run: DONE 2026-08-26 morning at 459/0 on the tree at `063c373`** (`/usr/bin/ruby tools/test_contract.rb`, banner `ruby 2.6.10 (universal.arm64e-darwin25)`, output in `build/run26.out`, which `.gitignore:29` ignores) — **and it is now OWED AGAIN for the owed-10 work**, which has never run under 2.6. A 2.6 run cannot be done through the bridge on any machine: it needs `/usr/bin/ruby` in Andriy's own terminal. |
