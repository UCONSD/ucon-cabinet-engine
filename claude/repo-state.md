...
| **Pushed** | **YES, 2026-08-25 late evening.** `refs/heads/main` and `refs/remotes/origin/main` are both **`da758d9`**; four commits went out in one push (`e01f2ef..da758d9`). The working tree was clean at that moment. **Do not trust this cell — read the two refs** (rule 16, and it is two `cat`s). |
| **Working tree** | **CLEAN at the push**, then this one line. B6 is whole, run in SketchUp and committed; `src/ucon_cabinet_engine/core/08_project.rb` is in `HEAD`. **Read it from `git`, never from this cell** — through the device mount `git status` can lag a write by a second, and the first list this row was given was one file short of the truth. |
